...
let nodeIdIndex = 0;
const buildNode = (node, level, oriLevel, parent, idPrefix, levelClassPrefix, scrollElement) => {
...
const setTimer = function(fn, delay, next) {
  let timer;
  let start = Date.now();
  const loop2 = () => {
...
const debounce = function(fn, delay = 16) {
  let id;
  const debounced = function() {
    ...
    if (id)
      clearTimer(id);
    id = setTimer(() => fn.apply(context, args), delay, (nextId) => id = nextId);
  };
...
const throttle = function(fn, duration = 16) {
  let time = null;
  return function() {
    const context = this;
    const args = arguments;
...
const getScrollTop = function(el) {
  return el === window ? el.pageYOffset : el === document ? document.scrollingElement.scrollTop : el.scrollTop;
};
...
const scrollTo = function(el, top, duration = 200, finished) {
  if (isScroll(el)) {
...
const slide = function(node, { delay = 0, duration = 150, easing = cubicOut } = {}) {
  ...
  return {
    ...
    duration,
    easing,
    css: (t) => `overflow: hidden;opacity: ${Math.min(t * 20, 1) * opacity};height: ${t * height}px;`
  };
};
...
const active = {
  ...
  unHoverd() {
    _acitve.update((v) => {
      return __spreadProps(__spreadValues({}, v), { hoverd: null });
    });
  },
  hoverd(el) {
    _acitve.update((v) => {
      return __spreadProps(__spreadValues({}, v), { hoverd: el });
    });
  }
};
const hightlight = derived(_acitve, ($active) => {
...
function create_if_block(ctx) {
  ...
  return {
    ...
    m(target, anchor) {
      ...
      if (!mounted) {
        dispose = [
          listen(div, "introend", ctx[9]),
          listen(div, "outroend", ctx[10])
        ];
        mounted = true;
      }
    },
...
function create_if_block_1(ctx) {
  ...
  return {
    ...
    p(ctx2, dirty) {
      const nav_changes = {};
      if (dirty & 1)
        nav_changes.nodes = ctx2[14].nodes;
      if (dirty & 1)
        nav_changes.parent = ctx2[14];
      if (dirty & 64)
...
function create_each_block(ctx) {
  ...
  let mounted;
  let dispose;
  let if_block = ctx[14].nodes && ctx[14].nodes.length && create_if_block_1(ctx);
  return {
    c() {
      a = element("a");
      t0 = text(t0_value);
      ...
      attr(a, "href", a_href_value = "#" + ctx[14].id);
      attr(a, "class", a_class_value = ctx[14].class);
      toggle_class(a, "active", ctx[7].node === ctx[14]);
    },
    m(target, anchor) {
      ...
      current = true;
      if (!mounted) {
        dispose = action_destroyer(handler_action = ctx[8].call(null, a, ctx[14]));
        mounted = true;
      }
    },
    p(new_ctx, dirty) {
      ctx = new_ctx;
      if ((!current || dirty & 1) && t0_value !== (t0_value = ctx[14].title + ""))
        set_data(t0, t0_value);
      if (!current || dirty & 1 && a_href_value !== (a_href_value = "#" + ctx[14].id)) {
        attr(a, "href", a_href_value);
      }
      if (!current || dirty & 1 && a_class_value !== (a_class_value = ctx[14].class)) {
        attr(a, "class", a_class_value);
      }
      if (handler_action && is_function(handler_action.update) && dirty & 1)
        handler_action.update.call(null, ctx[14]);
      if (dirty & 129) {
        toggle_class(a, "active", ctx[7].node === ctx[14]);
      }
      if (ctx[14].nodes && ctx[14].nodes.length) {
        if (if_block) {
...
function create_fragment$1(ctx) {
  let show_if = !ctx[11](ctx[7].node, ctx[4]);
  let if_block_anchor;
  let current;
  ...
  return {
    ...
    p(ctx2, [dirty]) {
      if (dirty & 144)
        show_if = !ctx2[11](ctx2[7].node, ctx2[4]);
      if (show_if) {
        if (if_block) {
...
function instance$1($$self, $$props, $$invalidate) {
  let $active;
  let $scrollLock;
  component_subscribe($$self, active, ($$value) => $$invalidate(7, $active = $$value));
  component_subscribe($$self, scrollLock, ($$value) => $$invalidate(12, $scrollLock = $$value));
  let { nodes = [] } = $$props;
  ...
  const handler = function(el, node) {
    node.element = el;
    if ($active.node === node) {
      set_store_value(active, $active = { node, el, hoverd: null }, $active);
    }
    let _heading = node;
    ...
    const click = function(e) {
      ...
      set_store_value(scrollLock, $scrollLock = true, $scrollLock);
      scrollTo(el2, _heading.y - scrollOffset, scrollDuration, function() {
        if (typeof onNav === "function") {
          onNav(e.target, _heading);
        }
        setTimer(() => {
          set_store_value(scrollLock, $scrollLock = false, $scrollLock);
        }, 16);
      });
    };
  ...
  const isCollapsed = function(activeNode, parentNode) {
    ...
    return parentNode.level + 1 > collapsedLevel && !lookup(activeNode, parentNode);
  };
  $$self.$$set = ($$props2) => {
    if ("nodes" in $$props2)
...
function create_fragment(ctx) {
  ...
  return {
    c() {
      ...
      div1 = element("div");
      create_component(nav.$$.fragment);
      attr(div0, "class", "bitoc-hightlight svelte-10e3kft");
      attr(div0, "style", ctx[5]);
      attr(div1, "class", "bitoc-navs svelte-10e3kft");
      attr(main, "class", "bitoc svelte-10e3kft");
    },
    m(target, anchor) {
...
function instance($$self, $$props, $$invalidate) {
  ...
  let $scrollLock;
  let $hightlight;
  component_subscribe($$self, active, ($$value) => $$invalidate(24, $active = $$value));
  component_subscribe($$self, scrollLock, ($$value) => $$invalidate(25, $scrollLock = $$value));
  component_subscribe($$self, hightlight, ($$value) => $$invalidate(5, $hightlight = $$value));
  ...
  const resolveHeadings = function() {
    ...
    const result = build(allHeadings, idPrefix, levelClassPrefix, realScrollElement);
  ...
  const fixed = function() {
    ...
    if (fixedOffset !== false && isDoc(realScrollElement)) {
      ...
      if (scrollElementTop >= _fixedOffset) {
        ...
        if (_fixedElement.classList.contains(fixedClassName)) {
          if (typeof beforeFixed == "function" && beforeFixed(false) === false) {
            return;
          }
  ...
  $$self.$$set = ($$props2) => {
    if ("contentElement" in $$props2)
      $$invalidate(8, contentElement = $$props2.contentElement);
    if ("scrollElement" in $$props2)
      $$invalidate(9, scrollElement = $$props2.scrollElement);
    if ("fixedElement" in $$props2)
      $$invalidate(10, fixedElement = $$props2.fixedElement);
    if ("headingSelector" in $$props2)
      $$invalidate(11, headingSelector = $$props2.headingSelector);
    if ("collapsedLevel" in $$props2)
      $$invalidate(0, collapsedLevel = $$props2.collapsedLevel);
    if ("idPrefix" in $$props2)
      $$invalidate(12, idPrefix = $$props2.idPrefix);
    if ("levelClassPrefix" in $$props2)
      $$invalidate(13, levelClassPrefix = $$props2.levelClassPrefix);
    if ("scrollDuration" in $$props2)
      $$invalidate(1, scrollDuration = $$props2.scrollDuration);
    if ("fixedOffset" in $$props2)
      $$invalidate(14, fixedOffset = $$props2.fixedOffset);
    if ("fixedClassName" in $$props2)
      $$invalidate(15, fixedClassName = $$props2.fixedClassName);
    if ("scrollOffset" in $$props2)
      $$invalidate(2, scrollOffset = $$props2.scrollOffset);
    if ("beforeFixed" in $$props2)
      $$invalidate(18, beforeFixed = $$props2.beforeFixed);
    if ("afterFixed" in $$props2)
      $$invalidate(19, afterFixed = $$props2.afterFixed);
  };
  {
    resolveHeadings();
  }
